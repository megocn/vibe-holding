...
<p align="center">
  ...
  <span style="color:#8B1A1A">新时代的基建维基百科</span>
</p>

<p align="center">
  一个开源的 <strong>AI 基建选型知识库</strong>，外加一个用它的<strong>桌面 / Web 客户端</strong>。<br/>
  779 个工具条目、953 条关系边，覆盖 Coding Agent → 大模型 → 框架 → 云与部署 → 数据库 → 支付 → 分发 的整条链路。<br/>
  它记的不只是工具，更是工具<strong>之间</strong>的关系：谁能平替谁、国内对标是哪个、哪两个放一起会打架。<br/>
...
<p align="center">
  ...
  <img alt="entries" src="https://img.shields.io/badge/条目-779-8B1A1A?style=flat-square" />
  <img alt="edges" src="https://img.shields.io/badge/关系边-953-2F5D62?style=flat-square" />
  <img alt="vendors" src="https://img.shields.io/badge/厂商-397-5C4033?style=flat-square" />
...
要把一个 Vibe Coding 的想法真正**跑起来、上线、收钱**，你得先把一整条基建链路都选一遍。而这条链路上的信息散落在官网、推文、测评和踩坑帖里，每周变价、变能力、变政策，国内外方案还各说各话。

墨台把这条链路整理成一份**结构化、有出处、可遍历**的知识库，并配了一个客户端来用它。它不是 Awesome List 或导航站那种「一堆链接」—— 那类资源**只见点，不见网**：看得见单个工具，看不清替代、依赖、搭配与冲突。墨台把「边」当成一等公民。

| 你想…… | 打开墨台的 |
| --- | --- |
| 查一个工具靠不靠谱、怎么收费、国内能不能直连 | **知识库** |
| 知道它的平替、国内对标、常见搭配 | **图谱**（或详情页的关联面板） |
| 从零搭一套栈（出海 SaaS、国内双端、RAG…） | **方案** 模板与选型向导 |
| 把 2–4 个候选摊开看差异 | **对比** |
| 跟上「最近谁又变价 / 发版 / 改政策了」 | **情报** |
| 管好一堆 API Key 与账号 | **凭据**（仅桌面，本机加密） |
| 搞明白「排行榜权重凭什么这么定」 | **[选型擂台](#选型擂台--群英论剑)** —— 把排序打成一场架 |
...
<p align="center">
  <img src="docs/screenshots/desktop/01-home.webp" width="100%" alt="墨台桌面端首页" /><br/>
  <em>桌面端首页 · 续读航线、内容脉搏与本周对垒</em>
</p>
...
目标不止是「把碎片查清楚」，而是逐步长成「选型 + 情报 + 本地凭据」一体的个人基建驾驶舱：你在哪一层拿不准，就在哪一层沿着边走下去。设计气质走「墨图 · Ink Atlas」—— 宣纸为底、墨为字、朱砂点睛；温润其表，精确其里。
...
> **[vibeholding-arena.pages.dev](https://vibeholding-arena.pages.dev/pixel.html)** —— 打开就能看，不用登录。墨台首页的「像素论剑」按钮也指向这里。

「AI 时代的选型擂台」不只是一句口号：墨台真的做了一个擂台，把知识库里的**权威榜名次、token 单价、流行度**折算成角色属性，让模型档位在水墨像素战场上打一架。
...
8. **歇口气**：首页点「像素论剑」进 [选型擂台](#选型擂台--群英论剑)，选个擂台看两场，顺便搞明白榜单权重是怎么回事。
...
- ✅ 方案模板、选型向导、并观对比
- ✅ 情报时间线、关注与草稿确认队列
- ✅ 桌面壳 + 浏览器窄屏适配；内容库 **779 条目 / 953 边 / 22 类全覆盖**
- ✅ 选型擂台「群英论剑」：13 张擂台 / 三种战斗模式 / 造擂台与分享码（闭源，[在线可玩](https://vibeholding-arena.pages.dev/pixel.html)）
