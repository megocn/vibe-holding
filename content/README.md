...
| 好例子 | `VS Code 系 · Agent/多文件编辑 · Rules 与 MCP` |
| 坏例子 | `AI 原生代码编辑器`（品类简介）；`适合日常写码的团队…`（适用场景，不是特点）；`本机 IDE 主力` / `Flavor A`（内部分类或未引用排位） |
| 禁止 | 只重复 `name`；「适合/需要…时优先」受众句；编造排行；把 LLM 产品族与档位混谈；内部代号（如 Flavor A）与无出处「主力」 |
